...
export default function Page() {
  ...
  const loadSeats = useCallback(() => {
    fetch(`${API}/api/events/${EVENT_ID}/seats`)
      .then((r) => r.json())
  ...
  // Seat map polling + orders, only once authenticated.
  useEffect(() => {
    if (!auth) return;
    loadSeats();
    loadOrders();
  ...
  const book = async () => {
    ...
    const res = await authFetch(`${API}/api/bookings`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ event_id: EVENT_ID, seat_ids: selected }),
    });
    ...
    <main style={{ maxWidth: 860, margin: "0 auto", padding: "40px 20px 80px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          <h1 style={{ fontSize: 34, fontWeight: 500, margin: "6px 0 2px" }}>Live in Bangkok 2026</h1>
          ...
          <>
            <div style={{ flex: 1, minWidth: 200 }}>
              <div style={{ fontSize: 13, color: "#9a978c" }}>
                เลือกแล้ว {selected.length}/{MAX_SEATS} ที่นั่ง
              </div>
...
function AuthForm({ onAuthed }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (ok) {
        onAuthed({ access: d.access_token, refresh: d.refresh_token, userId: d.user_id, email });
      } else {
